...
--]] local basic = require 'sunman/lib/basic'
local map = basic.map
local utf8chars = basic.utf8chars
local rime = require 'sunman/lib/rime'
local config = {}
config.encode_rules = {
    {length_equal = 2, formula = "AaAbBaBb"},
    {length_equal = 3, formula = "AaAbBaBbCaCb"},
    {length_equal = 4, formula = "AaAbBaBbCaCbDaDb"},
    {length_equal = 5, formula = "AaAbBaBbCaCbDaDbEaEb"},
    {length_equal = 6, formula = "AaAbBaBbCaCbDaDbEaEbFaFb"},
    {length_equal = 7, formula = "AaAbBaBbCaCbDaDbEaEbFaFbGaGb"},
    {length_equal = 8, formula = "AaAbBaBbCaCbDaDbEaEbFaFbGaGbHaHb"},
    {length_equal = 9, formula = "AaAbBaBbCaCbDaDbEaEbFaFbGaGbHaHbIaIb"},
    {length_equal = 10, formula = "AaAbBaBbCaCbDaDbEaEbFaFbGaGbHaHbIaIbJaJb"},
    {
        length_equal = 11,
...
-- options 要与方案保持一致
local options = {
    'sunman_spelling.off', 'sunman_spelling.lv1', 'sunman_spelling.lv2',
    'sunman_spelling.lv3'
}
options.default = 4

local processor = rime.make_option_cycler(options,
                                          'sunman_spelling/lua/cycle_key',
                                          'sunman_spelling/lua/switch_key')
...
local function get_tricomment(cand, env)
    ...
    if utf8.len(text) == 1 then
        ...
        return env.engine.context:get_option('sunman_spelling.lv1') and
                   xform(raw_spelling:gsub('%[(.-),.*%]', '[%1]')) or
                   env.engine.context:get_option('sunman_spelling.lv2') and
                   xform(raw_spelling:gsub('%[(.-,.-),.*%]', '[%1]')) or
                   xform(raw_spelling) -- xuma_spelling.lv3 is on
        --- 词组不显示注解
...
local function filter(input, env)
    if env.engine.context:get_option('sunman_spelling.off') then
    ...
    --]]
        if cand.type == 'simplified' and env.name_space == 'smsp_for_rvlk' then
            local comment = (get_tricomment(cand, env) or '') .. cand.comment
            cand = Candidate("simp_rvlk", cand.start, cand._end, cand.text,
                             comment)
        else
            ...
            if add_comment and add_comment ~= '' then
                ...
                cand.comment = cand.type ~= 'completion' and
                                   ((env.name_space == 'smsp' and env.is_mixtyping) or
                                   (env.name_space == 'smsp_for_rvlk')) and
